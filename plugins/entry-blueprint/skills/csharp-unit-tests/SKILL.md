...
- **Fixture-local conversion is part of test work; migrations are separate tasks.** Converting the legacy assertion and mocking calls in the fixture you touch is expected (see the legacy list). Changing the test runner, the harness, the layout or the integration profile is never a side effect of writing a test.
...
NUnit is also the standard runner. A project still on MSTest or xUnit (`[Fact]`, `[Theory]`, `[MemberData]`; the project notes list such projects) is migrated as a dedicated project-level task — runner, adapter, references and shared infrastructure together — never by converting one file during ordinary test work.
...
- **A fixture growing past roughly 250 lines is a prompt to inspect, not a limit.** First apply the [duplication ladder](#reducing-duplication) — parameterize, extract `Arrange` helpers, move data into builders, books and `*Source` classes. Only when distinct behaviour groups still make the fixture hard to navigate, split it into fixtures named by group (`EmployeeServiceAuthorizationFixture`, `EmployeeServiceUpdatesFixture`). Never split just to satisfy a line count. Length alone is not evidence of a production defect; but if the fixture is still hard to navigate after the whole ladder, and the behaviour groups you would split it into map onto distinct responsibilities of the subject, raise that about the production class — as an observation, not a refactor performed as part of test work.
...
- **Assert one logical outcome.** Two tightly-related properties of the same result are one concept and fine. **More than about two property assertions on one result: use `await Verify(...)`** on the projected contract — one assertion line, the whole contract pinned (see [Verify](#snapshot-testing-with-verifynunit)). Asserting unrelated outcomes is never fine — split the test, or collapse it with `[TestCase]`.
...
**Arrange and act helpers configure state or return values; they don't assert the outcome.** A `ShouldNotBeNull()` inside a capture helper reports the failure at the wrong place and hides the behaviour check from the test body. Return the captured value and let the test assert; throw only for a genuinely broken arrangement (a seed that could not be created), never for the behaviour the test exists to check.
...
- NUnit runs all of a fixture's tests on **one instance**, sequentially by default — that is what makes `[SetUp]`-assigned fields safe. Check the effective `[Parallelizable]` scope at assembly, fixture (including base classes) and method level before relying on it — a bare `[Parallelizable]` on a fixture is `ParallelScope.Self` and does not parallelize its tests. If per-test parallelism is ever genuinely needed, switch to `[FixtureLifeCycle(LifeCycle.InstancePerTestCase)]` first (one-time setup and teardown must then be `static`, inherited ones included), and remember that a fresh instance does not isolate static state or shared services.
- **An unconfigured interceptable member returns a Dummy when FakeItEasy can create one — otherwise `default(T)`, which may be `null`** (a non-virtual member of a faked class runs its real implementation instead). For typical fakeable reference types that means a non-null dummy (including things like `Expression` or `Type`), so a production `if (x == null)` branch is never reached unless you say so: `A.CallTo(() => _repository.FindById(id)).Returns(null)`. A test whose scenario *is* "not found" or "not set" must configure the null explicitly. Default return behaviour differs between mocking libraries, return types and configurations, so when converting a Moq or NSubstitute fixture configure every sentinel the scenario depends on explicitly — a ported test can keep compiling and silently start exercising a different path.
...
- A snapshot sees only what reaches the verified object and survives serialization and settings. A typed-DTO snapshot protects the deserialized shape; a wire-contract test that must catch an extra or renamed JSON field, a status code or a header snapshots the `HttpResponseMessage` through the project's registered HTTP converter (`Verify.Http`, initialized once) or an explicit projection of status, relevant headers and body text — and confirms the verified text contains the fields it protects.
...
- **Enum rules**: for a standalone `IsInEnum` cover every defined member as valid and representative undefined values as invalid — values beyond the defined range where the underlying type allows it, and gaps in sparse enums. When the rule adds a business restriction (a defined `Unknown` or `Deleted` member is rejected), classify the defined members by that restriction instead. For `[Flags]`, `IsInEnum` accepts combinations of the declared values; test unsupported bits according to the rule actually written.
- **A child validator gets its own fixture.** The parent fixture adds one canary proving the child is wired — one representative invalid child producing the nested failure path (`Child.Field`), and for a `.When(...)`-gated child that the condition applies. `ShouldHaveChildValidator(c => c.Child, typeof(ChildValidator))` only inspects the validator's structure and does not prove a condition lets the child run. Don't re-test the child's rules from the parent. `SetValidator` skips a `null` child, so when a missing child is invalid the parent needs its own `NotNull` rule and a test for it.
...
- **Edge inputs get a validity assertion in both directions.** A `Must`/`When` predicate or an inline nested-property rule that dereferences a sibling can throw on a null or empty input instead of validating it. Assert the intended failure for an invalid edge input and *successful validation* for a valid one — either assertion also surfaces an unexpected exception; merely asserting that the validator doesn't throw proves nothing about the outcome.
...
An integration test exercises the real components its contract depends on: API tests run the real pipeline and DI container through `WebApplicationFactory<Program>`, persistence tests may hit the database directly, side processes (worker, scheduler, importer) build their own host. Which persistence behaviour can be exercised is the project's **integration profile**, one of:

| Profile | Can exercise | Cannot exercise |
|---|---|---|
| **SQL Server** — Testcontainers.MsSql + Respawn (default for a new project whose tests must prove SQL Server persistence) | the application pipeline **and** SQL translation, constraints, transactions, migrations, persistence wiring | nothing by construction — only what the tests leave uncovered |
| **In-process host, no database** — dependencies replaced by fakes/stubs | the application pipeline: routing, DI, middleware, authorization, serialization, error translation; the only choice for an application without a database | anything about a database |
...
`TestCategory=unit` is normally safe solution-wide. A category filter silently skips fixtures that carry no category, and adapters other than NUnit's map the filter differently — the project notes list such projects if the repo has them. An unfiltered run also executes the integration tests, which need Docker or the local connection-string environment variables (SQL Server profile), any per-project secrets, and, where projects share a database, one project at a time — the project notes carry the specifics.
