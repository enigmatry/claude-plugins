...
- **Assert one logical outcome.** A few tightly-related properties of the same result are one concept and fine. Once the *shape* of a result is the contract — a DTO, a response, a document — `await Verify(...)` on the projected contract is the better trade than a growing stack of property assertions — one assertion line, the whole contract pinned (see [Verify](#snapshot-testing-with-verifynunit)). Asserting unrelated outcomes is never fine — split the test, or collapse it with `[TestCase]`.
...
`TestCategory=unit` is normally safe solution-wide. A category filter silently skips fixtures that carry no category, and adapters other than NUnit's map the filter differently — the project notes list such projects if the repo has them. **Before running anything that may select integration tests** — the unfiltered solution, a mixed project — read the prerequisites in the integration reference and the project notes (Docker or connection strings, secrets, which projects must not run concurrently).
