...
- Method naming: `clickX()`, `fillX(value)`, `selectX(value)`, `getX(): Locator` for assertions, and `armXCapture(uniqueKey): Promise<CreatedX>` for creates (see `data-teardown.md`).
