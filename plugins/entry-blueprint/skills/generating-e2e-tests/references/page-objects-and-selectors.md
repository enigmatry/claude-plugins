...
- **All static locators are `private readonly` fields assigned in the constructor** — never inline in a member body. The one exception is a dynamic locator that needs a runtime parameter, returned as a `Locator`:

```typescript
readonly getRow = (text: string): Locator => this.rows.filter({ hasText: text }).first();
```

- **Members are `readonly` arrow-function properties**, as `typescript` requires of every class — Playwright calls nothing on a page object by name, so none of the method-syntax carve-outs apply. Declare them after the locator fields they read.
- Scope dialog-only controls so they can't match the page behind the dialog: `page.getByRole('dialog').getByTestId(…)`.
- Member naming: `clickX()`, `fillX(value)`, `selectX(value)`, `getX(): Locator` for assertions, and `armXCapture(uniqueKey): CreateCapture` for creates — a one-line delegate to the project's capture util, never hand-rolled (see `data-teardown.md`).
